...
function loadProducts() {
  ...
  if (keyword.value.trim()) {
    request = searchProducts(keyword.value.trim(), currentPage.value, pageSize.value)
  } else if (tag.value) {
    request = listByTag(tag.value, currentPage.value, pageSize.value)
  } else {
    request = listProducts(currentPage.value, pageSize.value)
  }
...
watch(() => route.query, (query) => {
  ...
  tag.value = query.tag || ''
  tagLabel.value = getTagLabel(tag.value)
  currentPage.value = 0
  loadProducts()
}, { immediate: true })
</script>
